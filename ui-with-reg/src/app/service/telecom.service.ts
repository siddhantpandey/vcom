...
export class TelecomService {
  ...
  getBaseUrlForDeleteService(serviceId: number): string {
    return this.baseUrl + 'services/' + serviceId;
  }
  add(user: User): Observable<User> {
    return this.http
      .post(
        this.getBaseUrlForAdd(),
        JSON.stringify(user),
  ...
  getCustomerByEmailId(emid: string): Observable<User> {
    return this.http
      .get(this.getBaseUrlForCustomerByEmail(emid))
      .pipe(map(data => data.json()));
  }

  deleteServiceByServiceId(serviceId: number) {
    this.http
      .delete(this.getBaseUrlForDeleteService(serviceId))
      .pipe(map(data => data.json));
  }
}
